...
def assign_route_to_marker(lat, lon, routes):
    ...
    """
    marker_point = Point(lon, lat)  # (longitude, latitude)
    min_distance = float('inf')
    closest_route = "Route inconnue"
    for route in routes:
        line = LineString(route["coords"])  # Les coordonnées sont [lon, lat]
...
def create_map(center_lat, center_lon, bounds, display_path, marker_data=None,
    ...
    """
    Création de la carte avec option d'afficher ou non les routes et l'outil de dessin.
    Si marker_data est fourni, les marqueurs sont ajoutés.
    """
    if hide_osm:
    ...
    if draw_routes:
        for route in routes_ci:
            # Inversion des coordonnées : les routes sont stockées en [lon, lat], on souhaite [lat, lon]
...
##############################################
# Fonction utilitaire pour le jumelage par centre
##############################################
...
with tab_manuel:
    ...
    if uploaded_files_grand and uploaded_files_petit:
        if len(uploaded_files_grand) != len(uploaded_files_petit):
            ...
            # Tri par centre (pour jumelage cohérent)
            ...
            # Calcul des centres pour la carte
            ...
            ##############################################
            # Carte de dessin : TIFF GRAND (OSM masqué) avec routes et outil de dessin
            ##############################################
            ...
            # global_count permet d'avoir des ID uniques sur l'ensemble des paires
                ...
                for i, feature in enumerate(features):
                    if feature.get("geometry", {}).get("type") == "Point":
                        ...
                        if coords and isinstance(coords, list) and len(coords) >= 2:
                            # Les coordonnées sont extraites depuis le TIFF GRAND,
                            # on les reprojete sur le TIFF PETIT pour obtenir une position correspondante
                            ...
                            # Transformation vers le système UTM du TIFF PETIT
...
if st.session_state.pairs:
    ...
    if petit_bounds:
        center_lat_petit = (petit_bounds.bottom + petit_bounds.top) / 2
        center_lon_petit = (petit_bounds.left + petit_bounds.right) / 2
        # Pour la carte de suivi, on affiche les routes et les marqueurs
    ...
    # Si aucune paire n'est disponible, on affiche une carte par défaut avec les routes
    all_lons = []
    all_lats = []
    for route in routes_ci:
...
if global_markers:
    # Utilisation de pandas pour afficher toutes les colonnes, y compris "routes"
    df = pd.DataFrame(global_markers)
    st.dataframe(df)
else:
