#########################################
# (Vos imports et définitions de fonctions restent inchangés)
#########################################
...
# ... (Toutes vos fonctions déjà définies : extract_exif_info, latlon_to_utm, convert_to_tiff, etc.) ...

#########################################
# Gestion des missions et des variables globales
#########################################
# (Code de gestion des missions, initialisation de st.session_state, chargement des routes, etc.)
# Exemple :
...
#########################################
# Création et sélection de mission dans la sidebar
#########################################
...
# Préparation de global_markers (utilisé pour le dashboard et la carte de suivi)
global_markers = []
for markers in st.session_state.get("markers_by_pair", {}).values():
    global_markers.extend(markers)

#########################################
# Navigation entre Tableau de bord et Post-traitements/Détections
#########################################
nav_page = st.sidebar.radio("Navigation", ["Tableau de bord", "Post-traitements et Détections"])

if nav_page == "Tableau de bord":
    #########################################
    ...
    total_missions = len(st.session_state.missions)
    total_markers = len(global_markers)
    
    col1, col2 = st.columns(2)
    col1.metric("Nombre de missions", total_missions)
    col2.metric("Nombre total de marqueurs", total_markers)
    
    st.markdown("### Répartition des défauts par classe")
    if global_markers:
        df_markers = pd.DataFrame(global_markers)
        if "classe" in df_markers.columns and not df_markers["classe"].empty:
            df_counts = df_markers["classe"].value_counts().reset_index()
            df_counts.columns = ["Classe", "Nombre"]
            st.dataframe(df_counts)
            
            fig, ax = plt.subplots()
            couleurs = [class_color.get(cl, "#000000") for cl in df_counts["Classe"]]
            ax.bar(df_counts["Classe"], df_counts["Nombre"], color=couleurs)
            ax.set_ylabel("Nombre de défauts")
            ax.set_title("Distribution des défauts par classe")
            plt.xticks(rotation=45, ha="right")
            st.pyplot(fig)
        else:
            st.info("Aucune information de classe disponible dans les marqueurs.")
    else:
        st.info("Aucun marqueur à afficher.")

    st.markdown("### Répartition des niveaux de gravité")
    if global_markers:
        if "gravite" in df_markers.columns and not df_markers["gravite"].empty:
            df_gravite = df_markers["gravite"].value_counts().reset_index()
            df_gravite.columns = ["Gravité", "Nombre"]
            st.dataframe(df_gravite)
            
            fig2, ax2 = plt.subplots()
            ax2.pie(df_gravite["Nombre"], labels=df_gravite["Gravité"], autopct='%1.1f%%', startangle=90)
            ax2.set_title("Répartition par gravité")
            st.pyplot(fig2)
        else:
            st.info("Aucune information de gravité disponible dans les marqueurs.")
    else:
        st.info("Aucun marqueur à analyser pour la gravité.")

    st.markdown("### Historique des missions")
    if st.session_state.mission_history:
        # Ici, vous pouvez également afficher les défauts associés à chaque mission
        df_missions_dashboard = pd.DataFrame(st.session_state.mission_history)
        st.dataframe(df_missions_dashboard)
    else:
        st.info("Aucune mission enregistrée.")

    st.markdown("### Autres analyses")
    st.markdown("Vous pouvez ajouter ici d'autres graphiques ou analyses selon la variété de vos données.")

else:
    #########################################
    # Affichage de la partie "Post-traitements et Détections"
    #########################################
    st.title("TRAITEMENTS")
    
    #########################
    # Export de l'historique des missions (ZIP)
    #########################
    if st.session_state.mission_history:
        df_missions = pd.DataFrame(st.session_state.mission_history)
    ...
    #########################################
    # Bouton de sauvegarde et Export CSV
    #########################################
