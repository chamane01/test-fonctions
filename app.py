...
##############################################
# Fonctions de conversion et extraction EXIF
##############################################
...
##############################################
# Conversion JPEG → GeoTIFF & JPEG avec métadonnées de cadre
##############################################
def process_uploaded_image(uploaded_file, pixel_size):
    ...
    if lat is None or lon is None:
        return None  # On ignore l'image si pas de coordonnées GPS.
    ...
    # Pour ce traitement, nous ne calculons pas la direction de vol (flight angle) et le fixons à 0.
    flight_angle = 0

    # Configuration 1 : TIFF "petit" (scaling_factor = 1/5)
    ...
    # Configuration 2 : TIFF "grand" (scaling_factor = 1/3 et pixel_size multiplié par 2)
    ...
    # Configuration images : JPEG avec métadonnées de cadre (scaling_factor = 1)
    scaling_factor = 1
    ...
    # Ajout des métadonnées avec piexif
    try:
        import piexif
        if "exif" in img.info:
...
# 1. Téléversement unique des JPEG et conversion
...
# 2. Création des paires pour la détection manuelle
...
##############################################
# Paramètres pour la détection (routes, couleurs, etc.)
##############################################
class_color = {
...
##############################################
# Initialisation du session_state pour la détection manuelle
##############################################
if "current_pair_index" not in st.session_state:
    st.session_state.current_pair_index = 0
if "pairs" not in st.session_state:
    st.session_state.pairs = []  # Paires sous la forme {"grand":..., "petit":...}
if "markers_by_pair" not in st.session_state:
    st.session_state.markers_by_pair = {}

##############################################
# Interface en onglets pour la détection
##############################################
...
with tab_auto:
    ...
    if auto_uploaded_images:
        ...
        # Logique de détection automatique à implémenter ultérieurement.
    else:
...
##############################################
# Carte de suivi globale et récapitulatif
##############################################
