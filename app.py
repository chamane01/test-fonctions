...
st.title("Conversion & Détection d'anomalies")
...
# ---------------------------
# Fonctions de conversion (partie Conversion)
# ---------------------------
...
# Pour configuration images : conversion en JPEG avec métadonnées de cadre
def convert_to_jpeg_with_frame(image_bytes, pixel_size, utm_center, utm_crs, rotation_angle=0):
    ...
    # Calcul des coins pour ajout dans les métadonnées
    corners = [
    ...
    corner_coords = [tuple(np.around(transform_final * corner, 2)) for corner in corners]
    ...
    try:
        ...
        st.error("La librairie piexif est requise pour ajouter des métadonnées JPEG.")
        exif_bytes = None
...
# Fonction utilitaire pour sauvegarder des bytes dans un fichier temporaire
def save_bytes_to_tempfile(data_bytes, suffix):
...
# ---------------------------
# Fonctions de détection (partie Détection)
# ---------------------------
# Chargement des routes depuis un fichier JSON (à adapter selon votre contexte)
def load_routes():
    try:
        with open("routeQSD.txt", "r") as f:
            routes_data = json.load(f)
    except Exception as e:
        st.error("Erreur lors du chargement du fichier routeQSD.txt")
...
def assign_route_to_marker(lat, lon, routes):
    ...
    for route in routes:
        line = LineString(route["coords"])  # Coordonnées sous forme [lon, lat]
        distance = marker_point.distance(line)
        if distance < min_distance:
            min_distance = distance
            closest_route = route["nom"]
    threshold = 0.01  # Seuil en degrés
...
# Création de la carte avec ajout d'image, routes et outil de dessin
...
def create_map(center_lat, center_lon, bounds, display_path, marker_data=None,
    ...
    if marker_data:
        # Dictionnaires de couleurs et tailles
        class_color = {
...
###############################################################################
# SECTION 1 – CONVERSION JPEG → GeoTIFF & Export JPEG avec métadonnées de cadre
###############################################################################
st.header("1. Conversion")

# Initialisation du stockage en session pour les résultats de conversion
...
if uploaded_files:
    ...
    if len(images_info) == 0:
        ...
        # Bouton Configuration 1 : GeoTIFF en redimensionnement 1/5
        if st.button("Configuration 1 (Carte de suivi)"):
            config1_results = []
            for i, info in enumerate(images_info):
                # Calcul de l’angle de vol à partir de la trajectoire (si au moins 2 images)
                ...
                config1_results.append({
                    "filename": info["filename"].rsplit(".", 1)[0] + "_geotiff.tif",
                    "bytes": tiff_bytes,
                    "utm": info["utm"],
                    "utm_crs": info["utm_crs"]
                })
            st.session_state.conversion_results["config1"] = config1_results
            # Optionnel : proposer un téléchargement ZIP
            zip_buffer = io.BytesIO()
            with zipfile.ZipFile(zip_buffer, "w", zipfile.ZIP_DEFLATED) as zip_file:
                for item in config1_results:
                    zip_file.writestr(item["filename"], item["bytes"])
            zip_buffer.seek(0)
            st.download_button(
                label="Télécharger GeoTIFF Configuration 1 (ZIP)",
                data=zip_buffer,
                file_name="images_geotiff_config1.zip",
                mime="application/zip"
            )
        
        # Bouton Configuration 2 : GeoTIFF en redimensionnement 1/3 et résolution x2
        if st.button("Configuration 2 (Carte de dessin)"):
            ...
            for i, info in enumerate(images_info):
                ...
                config2_results.append({
                    "filename": info["filename"].rsplit(".", 1)[0] + "_geotiff_x2.tif",
                    "bytes": tiff_bytes,
                    "utm": info["utm"],
                    "utm_crs": info["utm_crs"]
                })
            st.session_state.conversion_results["config2"] = config2_results
            zip_buffer = io.BytesIO()
            with zipfile.ZipFile(zip_buffer, "w", zipfile.ZIP_DEFLATED) as zip_file:
                for item in config2_results:
                    zip_file.writestr(item["filename"], item["bytes"])
            zip_buffer.seek(0)
            st.download_button(
                label="Télécharger GeoTIFF Configuration 2 (ZIP)",
                data=zip_buffer,
                file_name="images_geotiff_config2.zip",
                mime="application/zip"
            )
        
        # Bouton Configuration images : JPEG avec métadonnées de cadre
        if st.button("Configuration images (Détection automatique)"):
            config_images_results = []
            for i, info in enumerate(images_info):
                if len(images_info) >= 2:
                    if i == 0:
                        dx = images_info[1]["utm"][0] - images_info[0]["utm"][0]
                ...
                config_images_results.append({
                    "filename": info["filename"].rsplit(".", 1)[0] + "_with_frame_coords.jpg",
                    "bytes": jpeg_bytes,
                    "utm": info["utm"],
                    "utm_crs": info["utm_crs"]
                })
            st.session_state.conversion_results["config_images"] = config_images_results
            zip_buffer = io.BytesIO()
            with zipfile.ZipFile(zip_buffer, "w", zipfile.ZIP_DEFLATED) as zip_file:
                for item in config_images_results:
                    zip_file.writestr(item["filename"], item["bytes"])
            zip_buffer.seek(0)
            st.download_button(
                label="Télécharger JPEG avec métadonnées (ZIP)",
                data=zip_buffer,
                file_name="images_with_frame_coords.zip",
                mime="application/zip"
            )
        
        st.success("Les résultats de conversion ont été générés et stockés en session. Vous pouvez maintenant passer à la détection.")

###############################################################################
# SECTION 2 – DÉTECTION (Utilise les fichiers issus de la conversion)
###############################################################################
st.header("2. Détection d'anomalies")

# Pour simplifier, nous utilisons ici les fichiers issus de la conversion stockés en session.
# La détection se scinde en deux onglets : Détection Automatique et Détection Manuelle

tab_auto, tab_manuel = st.tabs(["Détection Automatique", "Détection Manuelle"])

# --- Détection Automatique (configuration images)
with tab_auto:
    st.subheader("Détection Automatique")
    if "conversion_results" in st.session_state and "config_images" in st.session_state.conversion_results:
        auto_images = st.session_state.conversion_results["config_images"]
        st.write("Les images issues de la configuration images sont disponibles pour la détection automatique :")
        for item in auto_images:
            st.image(item["bytes"], caption=item["filename"], use_column_width=True)
        st.info("La logique de détection automatique pourra s'appuyer sur ces images.")
    else:
        st.info("Aucun résultat de conversion 'Configuration images' n'est disponible.")

# --- Détection Manuelle (utilise config1 et config2)
with tab_manuel:
    st.subheader("Détection Manuelle")
    # Vérifier que les deux configurations nécessaires sont disponibles
    if ("conversion_results" in st.session_state and 
        "config1" in st.session_state.conversion_results and 
        "config2" in st.session_state.conversion_results):
        
        # On considère que les listes config1 et config2 sont de même longueur
        config1_list = st.session_state.conversion_results["config1"]
        config2_list = st.session_state.conversion_results["config2"]
        pair_count = len(config1_list)
        
        # Création des paires : chaque paire contient un fichier "grand" (config1) et un "petit" (config2)
        pairs = []
        for i in range(pair_count):
            pairs.append({"grand": config1_list[i], "petit": config2_list[i]})
        st.session_state.pairs = pairs
        
        # Pour chaque paire, on doit obtenir des infos géoréférencées à partir du GeoTIFF.
        # On définit ici une fonction qui lit un fichier temporaire à partir des bytes.
        def get_image_info_from_bytes(item):
            # Sauvegarde temporaire
            temp_path = save_bytes_to_tempfile(item["bytes"], "tif")
            with rasterio.open(temp_path) as src:
                bounds = src.bounds
                center_lon = (bounds.left + bounds.right) / 2
                center_lat = (bounds.bottom + bounds.top) / 2
            return {"path": temp_path, "center": (center_lon, center_lat), "bounds": bounds, "temp_original": temp_path}
        
        # Création des listes pour la paire courante
        grand_list = [get_image_info_from_bytes(pair["grand"]) for pair in pairs]
        petit_list = [get_image_info_from_bytes(pair["petit"]) for pair in pairs]
        
        # Navigation entre paires
        if "current_pair_index" not in st.session_state:
            st.session_state.current_pair_index = 0
        col_nav1, col_nav2, col_nav3 = st.columns([1, 2, 1])
        prev_pressed = col_nav1.button("← Précédent", key="prev_pair")
        next_pressed = col_nav3.button("Suivant →", key="next_pair")
        ...
        current_pair = {"grand": grand_list[current_index], "petit": petit_list[current_index]}
        
        # Traitement de la paire courante : génération d'images PNG pour affichage sur les cartes
        ...
        temp_png_grand = save_bytes_to_tempfile(image_grand.tobytes(), "png")
        ...
        temp_png_petit = save_bytes_to_tempfile(image_petit.tobytes(), "png")
        image_petit.save(temp_png_petit)
        display_path_petit = temp_png_petit
        
        # Affichage de la carte de dessin (pour placer des marqueurs)
        st.subheader("Carte de dessin")
        map_placeholder_grand = st.empty()
        m_grand = create_map(
            center_lat= current_pair["grand"]["center"][1],
            center_lon= current_pair["grand"]["center"][0],
            bounds=grand_bounds,
            ...
            for i, feature in enumerate(features):
                if feature.get("geometry", {}).get("type") == "Point":
                    ...
                    if coords and isinstance(coords, list) and len(coords) >= 2:
                        ...
                        utm_x_petit, utm_y_petit = transform("EPSG:4326", current_pair["petit"]["bounds"].crs.to_string(), [new_lon], [new_lat])
        ...
        if st.session_state.pairs:
            # Utilisation de la première paire pour obtenir les bornes de la carte "petit"
            first_pair = st.session_state.pairs[0]
            try:
                with rasterio.open(save_bytes_to_tempfile(first_pair["petit"]["bytes"], "tif")) as src:
                    petit_bounds = src.bounds
            except Exception as e:
            ...
            if petit_bounds:
                ...
                m_petit = create_map(
                    ...
                    bounds=petit_bounds,
                    display_path=display_path_petit,
            ...
            st.info("Aucune donnée de détection manuelle n'est disponible.")
    else:
        st.info("Les résultats de conversion 'Configuration 1' et 'Configuration 2' ne sont pas disponibles.")

# Fin de la détection – Vous pouvez compléter ou adapter le nettoyage des fichiers temporaires
st.markdown("### Fin de la détection")
