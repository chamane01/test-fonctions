...
if uploaded_file is not None:
    ...
    # Création du canvas avec l'image en fond
    ...
    # Vérifier si l'utilisateur a cliqué (données de dessin disponibles)
    if canvas_result.json_data is not None:
        objects = canvas_result.json_data.get("objects", [])
        if len(objects) > 0:
            # Récupérer le dernier point cliqué
            last_point = objects[-1]
            x = int(last_point["left"])
            y = int(last_point["top"])
            st.write(f"Coordonnées sélectionnées : x={x}, y={y}")
            
            # Vérifier que les coordonnées sont dans les dimensions de l'image
            if x < image_np.shape[1] and y < image_np.shape[0]:
                # Extraction de la couleur au point sélectionné
                color = image_np[y, x]
